...
def record(episode,
    ...
    print(
        ...
        f"Loss: {int(total_loss / float(num_steps+1) * 1000) / 1000} | "
        f"Steps: {num_steps} | "
        f"Worker: {worker_idx}"
    )
...
class MasterAgent():
    def __init__(self):
        ...
        env = tron.Tron()
        self.state_size = reduce(operator.mul, env.observation_space.shape[1:], 1)
...
class Worker(threading.Thread):
    ...
    def run(self):
        ...
        while Worker.global_episode < args.max_eps:
            current_state = self.env.reset().reshape((2,-1))
            mem.clear()
            ep_reward = 0.
            ep_steps = 0
            self.ep_loss = 0
            ...
            while not done:
                ...
                action_0 = np.random.choice(self.action_size, p=probs.numpy()[0])
                action_1 = np.random.choice(self.action_size, p=probs.numpy()[1])
                new_state, (reward_0, reward_1), done = self.env.step(action_0, action_1)
                if new_state is not None:
                    new_state = new_state.reshape((2,-1))
                if done:
                    # reward_0 = -1
                    pass
                ep_reward += reward_0 + reward_1 # ONLY TAKE FIRST ONE
                mem.store(current_state[0], action_0, reward_0)
                mem.store(current_state[1], action_1, reward_1)
                ...
                if time_count == args.update_freq or done:
                    ...
                    with tf.GradientTape() as tape:
                        total_loss = self.compute_loss(done,
                                                       new_state[0] if new_state is not None else None,
                                                       mem,
                                                       args.gamma)
